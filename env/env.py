...
class Env_Generate(object):
    def __init__(self, num_APs, num_AP_antennas, num_RIS_elements, num_users, area_size, num_UE_antennas, num_RIS):
        self.L = num_APs
        ...
        self.Phi = np.diag(np.exp(1j * 2 * np.pi * np.random.rand(self.R * self.N)))
    ...
    def _position_generate_(self):
        ...
        self.RIS_position = np.random.uniform(0, self.area_size, (self.R, 2))
        ...
        for i in range(self.L):
            for r in range(self.R):
                self.disAP2RIS[i, r] = np.sqrt(
                    np.sum((self.RIS_position[r, :] - self.AP_position[i, :]) ** 2)
                    + (self.RIS_height - self.AP_height) ** 2
                )
    ...
    def _channel_G_generate(self, dis):
        ...
        disAP2RIS = np.sqrt(2e-3 * (dis ** (-2.2)))
