...
class FL_MADDPG(object):
    def __init__(
        ...
        self.actor_optimizer = [self.actor_optimizer_l for _ in range(num_fuzzy)]

        self.critic_optimizer_l = torch.optim.Adam(self.critic_l.parameters(), lr=critic_lr, weight_decay=critic_decay)
        self.critic_optimizer = [self.critic_optimizer_l for _ in range(num_fuzzy)]
        # self.actor = torch.zeros(num_fuzzy)
        # self.critic = torch.zeros(num_fuzzy)
        # self.actor_target = torch.zeros(num_fuzzy)
        # self.critic_target = torch.zeros(num_fuzzy)
        # self.actor_optimizer = torch.zeros(num_fuzzy)
    ...
    def update_parameters(self, num_fuzzy, replay_buffer, batch_size=16):
        for l in range(num_fuzzy):
            ...
            # next_action_old = self.actor_target[l](next_state.reshape(num_fuzzy * batch_size, -1)).reshape(batch_size,
            #                                                                                             num_fuzzy, -1)
